...
if (alphaTab.Environment.isRunningInWorker) alphaTab.Environment.initializeWorker();
	...
	if (alphaTab.Environment.webPlatform === alphaTab.WebPlatform.BrowserModule || alphaTab.Environment.isWebPackBundled || alphaTab.Environment.isViteBundled) {
		...
		try {
			return new alphaTab.Environment.alphaTabWorker(new alphaTab.Environment.alphaTabUrl("./alphaTab.worker.js", import.meta.url), { type: "module" });
		} catch (e) {
			alphaTab.Logger.debug("AlphaTab", "ESM webworker construction with direct URL failed", e);
		}
		let workerUrl = "";
		try {
			workerUrl = new alphaTab.Environment.alphaTabUrl("./alphaTab.worker.js", import.meta.url);
	...
	if (alphaTab.Environment.webPlatform === alphaTab.WebPlatform.BrowserModule || alphaTab.Environment.isWebPackBundled || alphaTab.Environment.isViteBundled) {
		...
		return alphaTabWorklet.addModule(new alphaTab.Environment.alphaTabUrl("./alphaTab.worklet.js", import.meta.url));
	}
